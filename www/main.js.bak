...
function renderStars(stars) {
  ...
  for (let i = 0; i < 5; i++) {
    if (i < full) {
      // ★ Full gold star
      html += `<svg width="20" height="20" viewBox="0 0 24 24" style="display:inline-block; vertical-align:middle;">
      ...
      // ☆ Half star: left gold, right transparent, gray outline all around
      html += `<svg width="20" height="20" viewBox="0 0 24 24" style="display:inline-block; vertical-align:middle;">
        <defs>
          <linearGradient id="halfGrad_${i}">
            <stop offset="50%" stop-color="#f5b342" />
            <stop offset="50%" stop-color="transparent" />
          </linearGradient>
        </defs>
        <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" fill="url(#halfGrad_${i})" stroke="#ddd" stroke-width="1.5"/>
      </svg>`;
    } else {
